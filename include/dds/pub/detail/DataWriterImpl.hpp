...
DataWriter<T, DELEGATE>::DataWriter(const dds::pub::Publisher &pub,
    ...
    : dds::core::Reference<DELEGATE<T>>(new DELEGATE<T>(pub, topic, qos)) {
...
void DataWriter<T, DELEGATE>::write(const FWIterator &begin,
...
template <typename T, template <typename Q> class DELEGATE>
DataWriter<T, DELEGATE> &DataWriter<T, DELEGATE>::unregister_instance(
    const ::dds::core::InstanceHandle &i, const dds::core::Time &timestamp) {
...
template <typename T, template <typename Q> class DELEGATE>
T &DataWriter<T, DELEGATE>::key_value(T &sample,
                                      const ::dds::core::InstanceHandle &h) {
...
dds::pub::detail::DataWriter<T>::DataWriter(
    const dds::pub::Publisher &pub, const ::dds::topic::Topic<T> &topic,
    const dds::pub::qos::DataWriterQos &qos)
    : ::org::eclipse::cyclonedds::pub::AnyDataWriterDelegate(qos, topic),
      pub_(pub), topic_(topic) {
        ...
        if (dds::topic::is_topic_type<T>::value == 0) {
                ISOCPP_THROW_EXCEPTION(ISOCPP_PRECONDITION_NOT_MET_ERROR,
                                       "DataWriter cannot be created, topic "
                                       "information not found");
        }
        DDSCXX_WARNING_MSVC_ON(6326)

        org::eclipse::cyclonedds::pub::qos::DataWriterQosDelegate dwQos =
            qos.delegate();

        dds_entity_t ddsc_pub =
            static_cast<dds_entity_t>(pub.delegate()->get_ddsc_entity());
        dds_entity_t ddsc_topic =
            static_cast<dds_entity_t>(topic.delegate()->get_ddsc_entity());
        ...
        dds_entity_t ddsc_writer = dds_create_writer(
            ddsc_pub, ddsc_topic, ddsc_qos,
            org::eclipse::cyclonedds::core::make_noop_listener().get());
        dds_delete_qos(ddsc_qos);
        ISOCPP_DDSC_RESULT_CHECK_AND_THROW(ddsc_writer,
...
template <typename T> dds::pub::detail::DataWriter<T>::~DataWriter() {
        if (!this->closed) {
                try {
                        this->close();
                } catch (...) {
                        /* Empty: the exception throw should have already traced
                         * an error. */
                }
        }
}
...
template <typename T>
bool dds::pub::detail::DataWriter<T>::is_loan_supported() {
        this->check();
        return AnyDataWriterDelegate::is_loan_supported(
            static_cast<dds_entity_t>(this->ddsc_entity));
}

template <typename T> T &dds::pub::detail::DataWriter<T>::loan_sample() {
        T *sample;
        this->check();
...
template <typename T>
void dds::pub::detail::DataWriter<T>::return_loan(T &sample) {
        this->check();
        AnyDataWriterDelegate::return_loan(
            static_cast<dds_entity_t>(this->ddsc_entity), &sample);
}

template <typename T>
void dds::pub::detail::DataWriter<T>::write_cdr(
...
void dds::pub::detail::DataWriter<T>::write_cdr(
    ...
    const dds::core::Time &timestamp) {
        ...
        AnyDataWriterDelegate::write_cdr(
            static_cast<dds_entity_t>(this->ddsc_entity), &sample,
            dds::core::InstanceHandle(dds::core::null), timestamp);
}

template <typename T>
void dds::pub::detail::DataWriter<T>::dispose_cdr(
    const org::eclipse::cyclonedds::topic::CDRBlob &sample) {
        this->check();
...
template <typename T>
void dds::pub::detail::DataWriter<T>::unregister_instance_cdr(
    const org::eclipse::cyclonedds::topic::CDRBlob &sample) {
        this->check();
...
template <typename T>
void dds::pub::detail::DataWriter<T>::write(const T &sample) {
        this->check();
...
void dds::pub::detail::DataWriter<T>::write(
    const T &sample, const ::dds::core::InstanceHandle &instance) {
        this->check();
        AnyDataWriterDelegate::write(
            static_cast<dds_entity_t>(this->ddsc_entity), &sample, instance,
            dds::core::Time::invalid());
}
...
void dds::pub::detail::DataWriter<T>::write(
    const dds::topic::TopicInstance<T> &i) {
        this->check();
        AnyDataWriterDelegate::write(
            static_cast<dds_entity_t>(this->ddsc_entity), &i.sample(),
            i.handle(), dds::core::Time::invalid());
}

template <typename T>
void dds::pub::detail::DataWriter<T>::write(
    const dds::topic::TopicInstance<T> &i, const dds::core::Time &timestamp) {
        this->check();
        AnyDataWriterDelegate::write(
            static_cast<dds_entity_t>(this->ddsc_entity), &i.sample(),
            i.handle(), timestamp);
...
template <typename T>
void dds::pub::detail::DataWriter<T>::writedispose(const T &sample) {
        this->check();
...
void dds::pub::detail::DataWriter<T>::writedispose(
    const T &sample, const dds::core::Time &timestamp) {
        this->check();
        AnyDataWriterDelegate::writedispose(
            static_cast<dds_entity_t>(this->ddsc_entity), &sample,
            dds::core::InstanceHandle(dds::core::null), timestamp);
...
template <typename T>
template <typename FWIterator>
void dds::pub::detail::DataWriter<T>::writedispose(const FWIterator &begin,
                                                   const FWIterator &end) {
        FWIterator b = begin;
        while (b != end) {
                this->writedispose(*b);
...
template <typename FWIterator>
void dds::pub::detail::DataWriter<T>::writedispose(
    const FWIterator &begin, const FWIterator &end,
    const dds::core::Time &timestamp) {
        FWIterator b = begin;
        while (b != end) {
...
template <typename SamplesFWIterator, typename HandlesFWIterator>
void dds::pub::detail::DataWriter<T>::writedispose(
    const SamplesFWIterator &data_begin, const SamplesFWIterator &data_end,
    const HandlesFWIterator &handle_begin,
    const HandlesFWIterator &handle_end) {
        SamplesFWIterator data = data_begin;
        HandlesFWIterator handle = handle_begin;
...
void dds::pub::detail::DataWriter<T>::writedispose(
    const SamplesFWIterator &data_begin, const SamplesFWIterator &data_end,
    const HandlesFWIterator &handle_begin, const HandlesFWIterator &handle_end,
    const dds::core::Time &timestamp) {
        SamplesFWIterator data = data_begin;
        HandlesFWIterator handle = handle_begin;
...
template <typename T>
const ::dds::core::InstanceHandle
dds::pub::detail::DataWriter<T>::register_instance(
    const T &key, const dds::core::Time &timestamp) {
        ...
        dds::core::InstanceHandle handle(
            AnyDataWriterDelegate::register_instance(
                static_cast<dds_entity_t>(this->ddsc_entity), &key, timestamp));
        return handle;
}
...
void dds::pub::detail::DataWriter<T>::unregister_instance(
    ...
    const dds::core::Time &timestamp) {
        ...
        AnyDataWriterDelegate::unregister_instance(
            static_cast<dds_entity_t>(this->ddsc_entity), handle, timestamp);
}
...
void dds::pub::detail::DataWriter<T>::unregister_instance(
    const T &sample, const dds::core::Time &timestamp) {
        ...
        this->check();
        AnyDataWriterDelegate::unregister_instance(
            static_cast<dds_entity_t>(this->ddsc_entity), &sample, timestamp);
}
...
dds::topic::TopicInstance<T> &dds::pub::detail::DataWriter<T>::key_value(
    dds::topic::TopicInstance<T> &i, const ::dds::core::InstanceHandle &h) {
        org::eclipse::cyclonedds::core::ScopedObjectLock scopedLock(*this);
        this->check();
        T sample;
        AnyDataWriterDelegate::get_key_value(
            static_cast<dds_entity_t>(this->ddsc_entity), &sample, h);
        i.handle(h);
        i.sample(sample);
        return i;
}
...
T &dds::pub::detail::DataWriter<T>::key_value(
    T &sample, const ::dds::core::InstanceHandle &h) {
        org::eclipse::cyclonedds::core::ScopedObjectLock scopedLock(*this);
        this->check();
        AnyDataWriterDelegate::get_key_value(
            static_cast<dds_entity_t>(this->ddsc_entity), &sample, h);
...
dds::pub::detail::DataWriter<T>::lookup_instance(const T &key) {
        org::eclipse::cyclonedds::core::ScopedObjectLock scopedLock(*this);
        this->check();
        dds::core::InstanceHandle handle(AnyDataWriterDelegate::lookup_instance(
            static_cast<dds_entity_t>(this->ddsc_entity), &key));
        return handle;
}
...
dds::pub::detail::DataWriter<T>::listener() const {
        org::eclipse::cyclonedds::core::ScopedObjectLock scopedLock(*this);
        this->check();
        return reinterpret_cast<dds::pub::DataWriterListener<T> *>(
            this->listener_get());
}

template <typename T> void dds::pub::detail::DataWriter<T>::close() {
        org::eclipse::cyclonedds::core::ScopedObjectLock scopedLock(*this);
...
void dds::pub::detail::DataWriter<T>::on_offered_deadline_missed(
    ...
    org::eclipse::cyclonedds::core::OfferedDeadlineMissedStatusDelegate &sd) {
        ...
        if (sr) {
                dds::pub::DataWriter<T, dds::pub::detail::DataWriter> dw =
                    wrapper();

                dds::pub::DataWriterListener<T> *l =
                    reinterpret_cast<dds::pub::DataWriterListener<T> *>(
                        this->listener_get());
                if (l)
